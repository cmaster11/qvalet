...
func (opt *noopOption) apply(_ *SNSHandler) {}
...
func (opt *authOption) apply(handler *SNSHandler) {
...
type certCacheDurationOption struct {
	certCacheDuration time.Duration
}

func WithCertCacheDuration(certCacheDuration time.Duration) Option {
	return &certCacheDurationOption{
		certCacheDuration: certCacheDuration,
	}
}

func (opt *certCacheDurationOption) apply(handler *SNSHandler) {
	handler.certCacheDuration = opt.certCacheDuration
}
